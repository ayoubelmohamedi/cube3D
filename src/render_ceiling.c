...
static void	calc_ceiling_position(t_player *player, t_ceil_render *cr, int y)
{
	...
	cr->current_ceil_x = player->x + cr->row_distance * (cr->player_dir_x
			- cr->plane_x);
	cr->current_ceil_y = player->y + cr->row_distance * (cr->player_dir_y
			- cr->plane_y);
...
static int	get_ceiling_color(t_player *player, t_ceil_render *cr)
{
	...
	{
		tex_pixel_ptr = player->env->texture->ceil_addr + (cr->tex_y
				* player->env->texture->ceil_line_len + cr->tex_x
				* (player->env->texture->ceil_bpp / 8));
...
void	render_ceiling(t_player *player, int screen_x, int y_start)
{
	...
	while (y < y_start)
	{
		...
		my_mlx_pixel_put(player->env, screen_x, y, ceil_fog_color(ceil_color,
				cr.row_distance));
		y++;
	}
}