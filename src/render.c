...
/*   Updated: 2025/05/10 19:40:31 by ael-moha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_ray(t_player *player, double ray_angle, int screen_x)
{
	...
	if (rays.mapX < 0 || rays.mapX >= player->env->map_width || rays.mapY < 0
		...
		return ;
	dist = dda_algo(&rays, &side, player);
	if (dist >= 0)
	{
		// fix bowelfish effect
		correct_dist = dist * cos(ray_angle - player->dir);
		if (correct_dist < 0.01)
			correct_dist = 0.01;
		wall_height = (int)(HEIGHT / correct_dist);
		if (wall_height < 0)
			wall_height = 0;
...
void	draw_vertical_line(t_player *player, t_vertical_info info)
{
	...
	if (player->env->has_texture && player->env->texture->has_ceiling)
		render_ceiling(player, info.x, y_start);
	else
